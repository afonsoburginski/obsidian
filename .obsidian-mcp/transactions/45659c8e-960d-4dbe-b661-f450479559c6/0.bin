...
> Domínio de monitoramento do [[ms-cameras]] (MOD-003 camera-health). Código: `apps/ms-cameras/src/health/` (motor 24/7) + `apps/ms-cameras/src/cameras/realtime/` (push ao vivo). Visual: [[03 - MOD-003 camera-health.excalidraw|diagrama]].
...
- **`streamStatus`** (`StreamHealthStatus`: `OK` / `DEGRADED` / `DOWN` / `INACTIVE`) - derivado no `IStreamDiagnostics` do streaming (UC-027), a partir do estado do path no mediamtx (frames em erro, reconexão, viewers sem peer). Ver [[Streaming - Diagnóstico de travamento no WebRTC]].
...
[[Integração com dispositivo]] · [[Streaming]] · [[Eventos, incidentes e alarmes]] · [[Status em tempo real]]
