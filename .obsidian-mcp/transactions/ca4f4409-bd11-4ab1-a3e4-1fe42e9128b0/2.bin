...
tags:
  ...
  - infra
aliases:
  - "Guia operacional (infra)"
atualizado: 2026-07-03
---

# Kubernetes - Guia operacional
...
Índice rápido: [provisionar um cluster](#provisionar-um-cluster-novo-deploy-manual) · [deploy contínuo (GitOps)](#deploy-contínuo-gitops) · [operação](#operação-comandos-comuns) · `kubernetes/docs/05-FALHAS-E-RECUPERACAO.md`.
...
| ------------ | ---------------------------------------------- | --------------------------------------------------------------------- | ------------------------------------- |
| 1. Cluster   | `terraform/rke2-kvm/` | VMs KVM/libvirt e RKE2 pinado, configurado no boot via cloud-init     | [[Kubernetes - Provisionamento (Terraform RKE2)|ver]] |
| 2. Addons    | `bootstrap/`                   | StorageClass default (local-path) e KEDA (autoescala de pods)         | [[Kubernetes - Bootstrap e KEDA|ver]]          |
| 3. Aplicação | `helm/attlas/`               | Chart com os microsserviços reais, Kafka, Kong, Postgres, Redis e web | [[Kubernetes - Deploy (Helm chart)|ver]]        |

As regras de escala de pods (ScaledObjects do KEDA) ficam em [[Kubernetes - Regras de escala (ScaledObjects)]] e são versionadas no chart.
...
> **Estado atual:** o deploy é manual, pelo `helm install` do passo 3. A distribuição por GitOps (ver `kubernetes/docs/04-CI-CD.md`) é o modelo-alvo e ainda não foi implementada: não existem a pasta `values/` nem o recurso GitRepo no repositório.
...
O modelo pretendido: cada cluster declara em Git a versão e os serviços que executa (`values/<cidade>.yaml`), e um controlador reconcilia o estado (`helm upgrade`), sem execução manual de `helm` no cluster. O desenho completo está em `kubernetes/docs/04-CI-CD.md`.
...
Adotar qualquer uma das duas exige criar a pasta `values/<cidade>.yaml`, migrar a CI para tags imutáveis e ativar o controlador. É um trabalho à parte, descrito em `kubernetes/docs/04-CI-CD.md`.
...
Para ajustar a escala de um serviço, edite o bloco `scaling:` do `values.yaml` e aplique com `helm upgrade`. Não edite o ScaledObject diretamente no cluster (`kubectl edit`): isso desalinha o estado da fonte de verdade em Git. As regras e a calibração de escala estão em [[Kubernetes - Regras de escala (ScaledObjects)]].
...
| ----------------------------------------------------------------------- | --------------------------------------------------- | --------------------- |
| `kubernetes/docs/01-VISAO-GERAL.md`                         | Plataforma; o que escala e o que não escala         | conceitual (todos)    |
| `kubernetes/docs/02-DIMENSIONAMENTO.md`                 | Teste de capacidade, métricas e cálculo do hardware | arquiteto e comercial |
| `kubernetes/docs/03-PRODUCAO.md`                               | Arquitetura de produção (topologia 3+3 HA, papéis)  | arquiteto e DevOps    |
| `kubernetes/docs/04-CI-CD.md`                                     | Distribuição da aplicação (build, push, GitOps)     | DevOps                |
| `kubernetes/docs/05-FALHAS-E-RECUPERACAO.md`       | Armadilhas, modos de falha e recuperação            | DevOps                |
| `kubernetes/docs/06-PROBLEMAS-IDENTIFICADOS.md` | Problemas em aberto (WebSocket sob escala de pods)  | DevOps e backend      |
| [[Kubernetes - Arquitetura]]                             | Diagrama Mermaid da infraestrutura                  | todos                 |
