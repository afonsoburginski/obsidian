...
1. **Banda provisionada (24/7)** = bitrate **configurado** lido do device (ONVIF universal; VAPIX nas Axis para pegar modo ABR/Zipstream), no healthcheck, **cacheado**. Fonte de verdade para planejamento e para o card de banda do VMS.
...
Healthcheck 24/7 (por câmera, já conectado)
   ...
   └─ Axis:  VAPIX param.cgi RateControl   → Mode + Max/TargetBitrate (enriquece)
        → grava "banda provisionada" (kbps) por perfil, cacheada  ── planejamento + VMS
...
5. **Banda do VMS** - `dashboard/bandwidth/bandwidth-monitoring.service.ts`: já soma `profile.bitrateKbps`; passa a somar o valor **device-truth** (agora fresco) e expõe `bitrateSource`/`updatedAt` para o front distinguir "provisionado" de "medido". (Remove o callout de "estático" do [[VMS - Banda e alertas]].)
