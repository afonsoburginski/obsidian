...
| RF-CAM-06 | Stream primário/secundário com codec/resolução/fps/bitrate configuráveis por stream | **Parcial** - model `CameraStreamProfile` existe (papel + config independente por stream), mas a **configuração não faz parte deste CRUD**; é feita na integração ONVIF/streaming ([[Streaming]] / [[Integração com dispositivo]]) |
