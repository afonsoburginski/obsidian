...
Outros grupos do mesmo controller pertencem a domínios vizinhos: `GET /cameras/video-wall` (UC-014 → [[VMS]]), `/:id/status` e realtime ([[Status em tempo real]]), `/:id/events[/:eventId]` e `/incidents` ([[Eventos, incidentes e alarmes]]), PTZ/`/presets`/`/automations` ([[PTZ e presets]]), `/:id/health` ([[Saúde e monitoramento]]), streaming/`/hls` ([[Streaming]]).
