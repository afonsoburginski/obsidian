...
tags:
  ...
  - infra
aliases:
  - "Bootstrap - KEDA e autoescala"
atualizado: 2026-07-03
---

# Kubernetes - Bootstrap e KEDA
...
| Arquivo                                  | O que é                                                                                   |
| ---------------------------------------- | ----------------------------------------------------------------------------------------- |
| `install.sh`             | **O código** - script idempotente que aplica os addons (local-path + KEDA via Helm)       |
| `keda-values.yaml` | **O arquivo de configuração** - values do chart `kedacore/keda` (réplicas, recursos, log) |
...
| **KEDA**                   | `2.16.1`             | escalonamento horizontal de pods (ScaledObjects) - ver `kubernetes/docs/01-VISAO-GERAL.md` seção 4.1 | **Helm** (`kedacore/keda`)                                         |

> [!note] Estado no servidor (19/06/2026)
> O **KEDA agora é uma release Helm** (`helm list -n keda` → `keda 2.16.1 deployed`). Foi originalmente aplicado via `kubectl apply` e depois **adotado pelo Helm** com `helm upgrade --install ... --take-ownership` (sem downtime); os deployments duplicados do manifesto antigo foram removidos. O **local-path** segue por manifesto (não tem ganho em Helm). Daqui pra frente o KEDA pode virar dependência do Fleet, reusando `keda-values.yaml`.
...
Sobe `keda-operator` + `keda-metrics-apiserver` + `keda-admission-webhooks` (todos `2.16.1`). Os **ScaledObjects por serviço** não entram aqui - vêm no chart da aplicação (`infra/helm/attlas`, bloco `scaling`); ver `kubernetes/docs/01-VISAO-GERAL.md` seção 4.1.
...
Companion: `kubernetes/docs/01-VISAO-GERAL.md` (infraestrutura), `kubernetes/docs/04-CI-CD.md` (distribuição da app), [[Kubernetes - Provisionamento (Terraform RKE2)]] (cluster), [[Kubernetes - Regras de escala (ScaledObjects)]] (regras de escala).
