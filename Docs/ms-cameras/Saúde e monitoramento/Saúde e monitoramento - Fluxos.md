...
> Passo a passo dos ciclos e consultas. Hub: [[Saúde e monitoramento]]. Como funciona por dentro: [[Saúde e monitoramento - Arquitetura e estratégias]]. Visual: [[03 - MOD-003 camera-health.excalidraw|diagrama]].
...
7. **Mudou o estado?** - se sim: abre/fecha incidente (duração no evento de restauração), grava no log de eventos e publica `CameraConnectivityHealthChangedEvent` → Redis + WebSocket ([[Status em tempo real]]). Se não, nada é publicado.
...
Aparece como **aba** na página de detalhe da câmera e como **modal** aberto pela InfoCam no VMS (tabs "Informações Gerais" | "Saúde da Câmera") - mesmo backend nos dois.
