...
> As **regras de negócio** da aba "Saúde da Câmera", lidas da planilha do cliente na Sprint 22. Nasceu como contexto compartilhado das tarefas [[SOFTWARE-1922 - Janelas de 5 min + latência + rollup 90d]], [[SOFTWARE-1923 - Bitrate histórico + TTFF]] e [[SOFTWARE-1924 - getHealthMetrics + série + SLA]], e ficou como referência do domínio. Implementação em [[Saúde e monitoramento]].
> Alvo: `GET /api/cameras/:id/health?period=7d|30d|90d` retornando `ICameraHealthMetrics`, que alimenta a aba inteira do Figma.

A "Saúde da Câmera" aparece como aba na página de detalhe e como modal aberto pela InfoCam no VMS (tabs "Informações Gerais" e "Saúde da Câmera"). Mesmo backend nos dois.
