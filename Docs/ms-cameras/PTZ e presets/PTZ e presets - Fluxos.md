...
Fluxos de use case (backend) e de usuário (frontend). Contexto e endpoints: [[PTZ e presets]]. Arquitetura: [[PTZ e presets - Arquitetura e estratégias]]. Visual: [[05 - MOD-004 ptz-presets.excalidraw|diagrama]].
...
Superfície principal do feature module `cameras` (frontend). Controles manuais (setas/joystick, zoom, speed) → endpoints PTZ; lista/gestão de presets e tours → endpoints de presets/automations; a **posição PTZ atual** atualiza via Socket.IO (`camera:ptz:position`, ver [[Status em tempo real]]). Criar preset captura a posição corrente; um clique em preset dispara `goto`.

## User flow - PTZ inline no VMS (RF-VW-04)

O controle PTZ fica disponível via overlay em qualquer célula do VMS, **sem trocar de tela**, e o **estado da sessão PTZ é mantido por célula ativa**. Os comandos usam os mesmos endpoints PTZ desta nota; a orquestração de layout/células pertence ao [[VMS]].
...
- [[PTZ e presets]] · [[PTZ e presets - Arquitetura e estratégias]] · [[PTZ e presets - Requisitos e SLA]]
- [[VMS]] · [[Status em tempo real]] · [[Integração com dispositivo]]
