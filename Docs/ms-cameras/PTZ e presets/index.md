...
Controle de câmeras móveis (Pan-Tilt-Zoom) do módulo Câmeras: movimentação manual, posições nomeadas (presets), tours/automações e rastreamento de posição em tempo real. Domínio do backend [[ms-cameras]]. Visual: [[05 - MOD-004 ptz-presets.excalidraw|diagrama]].
...
Detalhes de protocolo e integração de hardware: [[Integração com dispositivo]].
...
A auditoria de cada comando cai em `CameraEventLog` (`eventType='PTZ_COMMAND'`) - ver [[PTZ e presets - Arquitetura e estratégias]] e [[Eventos, incidentes e alarmes]]. A posição PTZ observada é gravada em `CameraOperationalSnapshot` (`ptzPan/ptzTilt/ptzZoom`) - ver [[Status em tempo real]].

## Notas deste domínio

- [[PTZ e presets - Arquitetura e estratégias]] - pipeline ONVIF/VAPIX, autorização, tour-runner, auditoria, posição em tempo real.
- [[PTZ e presets - Fluxos]] - comando PTZ, goto preset, tour; user flows (detalhe da câmera, VMS, mapa).
- [[PTZ e presets - Requisitos e SLA]] - RF/RNF cobertos, estado de implementação, SLA de latência.
- Relacionados: [[Integração com dispositivo]] · [[Status em tempo real]] · [[Eventos, incidentes e alarmes]] · [[VMS]].
