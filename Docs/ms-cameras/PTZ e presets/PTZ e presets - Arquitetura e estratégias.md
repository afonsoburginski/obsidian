...
Como o ms-cameras executa, autoriza, audita e observa comandos PTZ. Contexto geral: [[PTZ e presets]]. Visual: [[05 - MOD-004 ptz-presets.excalidraw|diagrama]].

## Dois caminhos de execução: ONVIF e VAPIX

O serviço `cameras/services/ptz.service.ts` expõe dois pipelines, escolhidos por **tipo de comando**, não por fabricante configurado. Integração de hardware detalhada em [[Integração com dispositivo]].
...
O consumidor Kafka de comandos de emergência (`attlas.emergencies.ptz-command`) está **declarado no SPEC mas SEM `@EventPattern` implementado** (confirmado: os únicos `@EventPattern` de ms-cameras são `EVENT_INGEST`, `EVENT_LOGGED` ×2 e `INCIDENT_CREATED`). Logo, prioridade máxima e preempção de sessão de operador por emergência **ainda não existem** no código. Ver [[Eventos, incidentes e alarmes]] para os consumidores Kafka ativos.
...
- Esses registros alimentam o log de eventos da câmera - ver [[Eventos, incidentes e alarmes]].
...
- Fluxo completo do tempo real em [[Status em tempo real]].

## Ver também

- [[PTZ e presets]] · [[PTZ e presets - Fluxos]] · [[PTZ e presets - Requisitos e SLA]]
- [[Integração com dispositivo]] · [[Status em tempo real]] · [[Eventos, incidentes e alarmes]]
