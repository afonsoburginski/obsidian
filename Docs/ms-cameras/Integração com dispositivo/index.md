...
| [[Saúde e monitoramento\|Saúde e monitoramento]] | Heartbeat/eventos via **clients** WS/PullPoint | `health/workers/camera-health.worker.ts` → `AxisWsClient` / `OnvifPullPointClient` |
| [[Cameras\|Cameras]] (CRUD) | Probe de credenciais no cadastro | `cameras/services/camera-credential-probe.service.ts` (ONVIF handshake) |
